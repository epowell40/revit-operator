...
4. A durable provider-neutral session coordinator that feeds the same iteration/diagnostic protocol back into OpenAI, Anthropic, and Gemini agent loops. The current EPIC-0444 resume contract is exposed through the local MCP general-agent lane; the separate one-shot provider runner remains fail-closed until it can verify the same receipts.
