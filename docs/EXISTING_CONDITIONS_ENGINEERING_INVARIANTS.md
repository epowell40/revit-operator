...
- Independent source-symbol and wall-gap evidence for the same projected physical interval collapse to one classification obligation, preferring the source-symbol/redacted-host observation. Adjacent openings and openings on materially separated parallel walls remain distinct, while wall-host ambiguity remains explicit elsewhere in the receipt.
