...
- Agent-selected plumbing routes/fixtures and electrical devices can begin in registered plan-image pixels with `npm run existing-conditions -- compile-registered-mep-observations --input <registered-pixels.json> --out <compilation.json> --package-out <observations.json>`. The adapter verifies the agent-visible render's hash, dimensions, registration, frame, coordinate bounds, discipline, evidence roles, and per-material evidence receipts before converting pixels into the existing structured MEP package. It never treats plotted panel/circuit labels as native circuit membership. When image alignment exposes repeated-building-module candidates, first run `npm run existing-conditions -- assess-registration-ambiguity --input <candidate-search.json> --out <ambiguity-receipt.json>` and attach the same `registration_ambiguity` input to the registered package. Compilation then fails unless the selected transform wins through at least three non-collinear, spatially independent, non-repeating, agent-visible control anchors with separately hash-bound local context, or a decisive normalized score margin over a materially distinct alternate transform using one common target-excluded evidence set. Near-duplicate local peaks do not count as alternate modules, and the ambiguity receipt does not authorize a native write.
