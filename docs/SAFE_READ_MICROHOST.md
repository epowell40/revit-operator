...
The deployment coordinator must deliver the receipt's SHA-256 and the package pin independently of the package and manifest. The receipt file itself must use one canonical absolute, non-reparse path outside the package, Revit Addins, and the complete OperatorDeploy managed product tree, including deployment state and sibling releases. Windows device namespaces and multi-link files are rejected. The same read handle supplies the receipt bytes and its resolved long DOS/UNC path, volume/file identity, link count, size, and write-time checks; if Windows or a remote filesystem cannot establish those facts, admission fails closed. OperatorDeploy requires the external values on install, update, repair, and bundle-only validation, copies the admitted receipt into the release, and persists its identity in state and the activation journal. It then explicitly enumerates the source package without following reparse points, independently recomputes package, proof, PE, runtime-attestation, manifest, and final-layout facts immediately before activation, and rechecks them during installed validation, repair, rollback, and crash recovery. A managed SafeRead release is exact: only the manifest, persisted receipt, all declared component files, and their implied directories may exist, and no ancestor or descendant from the product root through that release may be a reparse point. Optional zero-file components remain compatible directory-less no-ops.
