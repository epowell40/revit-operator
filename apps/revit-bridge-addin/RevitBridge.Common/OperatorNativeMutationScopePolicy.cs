    ...
    public static class OperatorNativeMutationScopePolicy
    {
            ...
            bool allowUnexpectedExistingForRollback = false)
        {
            ...
            if (affected.Count > maxAffectedElements)
            {
                ...
                decision.Error = $"native-api-mutation-ops affected {affected.Count} elements, exceeding transaction.maxAffectedElements={maxAffectedElements}; this rejection requires verified failure-processing rollback.";
                return decision;
            }
            if (!allowCreate && added.Length > 0)
            {
                decision.Code = "creation_not_allowed";
                decision.Error = $"native-api-mutation-ops created {added.Length} elements while transaction.allowCreate=false; this rejection requires verified failure-processing rollback.";
                return decision;
            }
            if (unexpectedExisting.Length > 0)
            {
                ...
                decision.Error = $"native-api-mutation-ops affected existing elements outside transaction.allowedExistingElementIds: {string.Join(",", unexpectedExisting.Take(16))}; this rejection requires verified failure-processing rollback.";
                return decision;
            }
