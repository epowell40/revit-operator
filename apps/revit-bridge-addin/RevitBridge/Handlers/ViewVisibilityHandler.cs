...
namespace RevitBridge.Handlers
{
        ...
        public sealed class Params
        {
            ...
            public long? linePatternId { get; set; } // projection line pattern id
            public string? linePatternName { get; set; } // projection line pattern by name
        ...
        private static void ApplyAction(Document doc, View view, string action, Params p)
        {
            switch (action)
            {
                ...
                case "set_scope_box":
                {
                    var scope = ResolveScopeBox(doc, p.scopeBoxId, p.scopeBoxName);
                    if (scope == null)
                        throw new InvalidOperationException("visibility.set_scope_box requires scopeBoxId or scopeBoxName.");

                    var set = TrySetBuiltInElementIdParameter(view, "VIEWER_VOLUME_OF_INTEREST_CROP", scope.Id);
                    if (!set) throw new InvalidOperationException("Unable to set scope box on this view.");
                    return;
                }
                case "clear_scope_box":
                {
                    var cleared = TrySetBuiltInElementIdParameter(view, "VIEWER_VOLUME_OF_INTEREST_CROP", ElementId.InvalidElementId);
                    if (!cleared) throw new InvalidOperationException("Unable to clear scope box on this view.");
                    return;
                }
                case "set_category_override":
                {
                    ...
                    if (ApplyProjectionLinePattern(doc, ogs, p.linePatternId, p.linePatternName))
                    {
                        changed = true;
                    }
                    ...
                    if (!changed)
                        throw new InvalidOperationException("visibility.set_category_override requires lineWeight and/or linePatternId|linePatternName and/or r,g,b.");
        ...
        private static LinePatternElement? ResolveLinePatternElement(Document doc, long? linePatternId, string? linePatternName)
        {
            if (linePatternId.HasValue && linePatternId.Value > 0)
            {
                return doc.GetElement(RevitBridge.Common.ElementIdCompat.Create(linePatternId.Value)) as LinePatternElement;
            }

            var name = (linePatternName ?? "").Trim();
            if (name.Length == 0) return null;

            return new FilteredElementCollector(doc)
                .OfClass(typeof(LinePatternElement))
                .Cast<LinePatternElement>()
                .FirstOrDefault(lp => (lp.Name ?? "").Trim().Equals(name, StringComparison.OrdinalIgnoreCase));
        }

        private static bool ApplyProjectionLinePattern(Document doc, OverrideGraphicSettings ogs, long? linePatternId, string? linePatternName)
        {
            var requested = (linePatternId.HasValue && linePatternId.Value > 0) || !string.IsNullOrWhiteSpace(linePatternName);
            if (!requested) return false;

            var pattern = ResolveLinePatternElement(doc, linePatternId, linePatternName);
            if (pattern == null)
            {
                throw new InvalidOperationException("visibility line pattern not found. Provide linePatternId or linePatternName.");
            }

            ogs.SetProjectionLinePatternId(pattern.Id);
            return true;
        }
        ...
        private static ResolvedRuleParameter? ResolveRuleParameter(Document doc, List<ElementId> categoryIds, string parameterNameOrBuiltIn)
        {
            ...
            foreach (var catId in categoryIds)
            {
                ...
                {
                    var p = ResolveParameterFromElement(element, token, builtIn);
                    if (p == null) continue;
            ...
            if (builtIn.HasValue)
            {
                return new ResolvedRuleParameter
                {
                    ParameterId = new ElementId((int)builtIn.Value),
        ...
        {
            var key = (raw ?? "equals").Trim().ToLowerInvariant();
            return key switch
            {
                "eq" => "equals",
                "=" => "equals",
        ...
        private static Element? TryGetScopeBox(Document doc, View view)
        {
            try
            {
                var bip = (BuiltInParameter)Enum.Parse(typeof(BuiltInParameter), "VIEWER_VOLUME_OF_INTEREST_CROP", ignoreCase: true);
