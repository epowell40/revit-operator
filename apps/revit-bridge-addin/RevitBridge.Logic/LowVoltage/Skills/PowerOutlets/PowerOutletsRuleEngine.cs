...
{
    public class PowerOutletsRuleEngine : ILowVoltageRuleEngine
    {
        public LayoutResult Evaluate(LayoutContext context)
        {
            var profile = context.DisciplineProfile as PowerOutletsProfile ?? PowerOutletsProfile.CreateDefault();
            ...
            {
                if (!profile.EndpointRequirements.TryGetValue(endpoint.EndpointClass, out var requirement))
                {
                ...
                var clusterDistance = requirement.ClusterDistanceFt > 0
                    ? requirement.ClusterDistanceFt
                    : profile.DuplicateSuppressionRules.DefaultClusterDistanceFt;
                if (selectedActions.Any(action =>
                        ...
                        && Geometry2D.Distance(existingEndpointLocation, endpoint.Location) < clusterDistance))
                {
                ...
                if (familyType == null)
                {
                    ...
                    {
                        AddReview(result, context.Diagnostics, "missing_family_symbol", "warning", "No power family symbol matched device category " + requirement.DeviceCategory + ".", endpoint.RoomId, "endpoint:" + endpoint.SourceElementId);
                    }
                }
                ...
                var winner = SelectBestCandidate(context, endpoint, room, requirement, candidates, selectedActions, profile);
                if (winner == null)
                {
                    AddSkippedEndpoint(context.Diagnostics, endpoint, "no_valid_candidates");
                    if (requirement.ReviewIfUnknownHost)
                    {
                        AddReview(result, context.Diagnostics, "host_ambiguity", "warning", "Endpoint " + endpoint.SourceElementId + " could not be resolved to a valid power placement.", endpoint.RoomId, "endpoint:" + endpoint.SourceElementId);
                    }
                    continue;
                }
                ...
                {
                    ActionId = "power-" + endpoint.SourceElementId,
            ...
            result.Assumptions.Add("Power outlet layout used profile '" + profile.OccupancyProfile + "' with deterministic endpoint-driven placement.");
            result.Assumptions.Add("Power endpoint normalization comes from the shared low-voltage normalization profile; unknown equipment is surfaced for profile expansion.");
            return result;
        }

        private static List<PowerEndpoint> DetectEndpoints(
            LayoutContext context,
            PowerOutletsProfile profile,
            IReadOnlyDictionary<long, RoomState> roomsById,
            LayoutResult result)
        {
            var endpoints = new List<PowerEndpoint>();
            var equipmentItems = context.State.Equipment
                .OrderBy(item => item.Id)
                .Select(item => new PowerEndpointSource
                {
            ...
            foreach (var source in equipmentItems.Concat(fixtureItems).OrderBy(item => item.SourceElementId))
            {
                ...
                var isUnknown = string.Equals(endpointClass, "unknown_power_endpoint", StringComparison.OrdinalIgnoreCase);
                ...
                if (isUnknown)
                {
                    ...
                    AddSkippedEndpoint(context.Diagnostics, new PowerEndpoint
                    {
                    ...
                    {
                        AddReview(result, context.Diagnostics, "unknown_power_endpoint", "warning", "Endpoint family " + (source.FamilyName ?? "unknown") + " / " + (source.TypeName ?? "unknown") + " requires mapping review for power outlets.", room?.Id, "endpoint:" + source.SourceElementId);
                    }
                    continue;
                }
                ...
                endpoints.Add(new PowerEndpoint
                {
                    SourceElementId = source.SourceElementId,
                    SourceKind = source.SourceKind,
                    EndpointClass = endpointClass,
        ...
        private static List<CandidatePoint> BuildCandidates(LayoutContext context, PowerEndpoint endpoint, RoomState room, PowerEndpointRequirement requirement)
        {
        ...
        private static CandidatePoint? SelectBestCandidate(
            ...
            RoomState room,
            PowerEndpointRequirement requirement,
            ...
            PowerOutletsProfile profile)
        {
            ...
            {
                var rejection = ValidateCandidate(context, endpoint, room, requirement, candidate, selectedActions, profile);
                if (rejection != null)
                {
                ...
                candidate.Score = ScoreCandidate(profile, endpoint, requirement, candidate);
            ...
            PowerOutletsProfile profile)
        {
            ...
            var minSeparation = requirement.MinSeparationFt > 0
                ? requirement.MinSeparationFt
                : profile.DuplicateSuppressionRules.DefaultMinSeparationFt;
            if (selectedActions.Any(action =>
                    action.RoomId == room.Id
                    && Geometry2D.Distance(action.Candidate.Location, candidate.Location) < minSeparation))
            {
        ...
        private static double ScoreCandidate(PowerOutletsProfile profile, PowerEndpoint endpoint, PowerEndpointRequirement requirement, CandidatePoint candidate)
        {
            var score = 0.0;
        ...
        private static FamilyTypeState? ResolveFamilyType(LayoutContext context, PowerOutletsProfile profile, string deviceCategory)
        {
            var tokens = new List<string>();
            if (profile.ReceptacleTypePreferences.TryGetValue(deviceCategory, out var configured))
            {
        ...
        private static int ScoreFamilyType(FamilyTypeState type, IEnumerable<string> tokens)
        {
            ...
            if (haystack.IndexOf("power", StringComparison.OrdinalIgnoreCase) >= 0) score += 5;
            if (haystack.IndexOf("receptacle", StringComparison.OrdinalIgnoreCase) >= 0) score += 5;
            if (haystack.IndexOf("device", StringComparison.OrdinalIgnoreCase) >= 0) score += 5;
            return score;
        }
        ...
        private static string ResolveEndpointClass(string? semanticType)
        {
            ...
            {
                return "unknown_power_endpoint";
            }

            return semanticType.Trim();
        }

        private static bool MatchesUnknownEndpointKeywords(PowerOutletsProfile profile, string searchableText)
        {
        ...
        private static void AddSkippedEndpoint(DiagnosticReport diagnostics, PowerEndpoint endpoint, string reason)
        {
        ...
        private sealed class PowerEndpointSource
        {
        ...
        private sealed class PowerEndpoint
        {
            public long SourceElementId { get; set; }
            public string SourceKind { get; set; } = string.Empty;
