...
namespace RevitBridge.Logic.Handlers
{
        ...
        public sealed class Params
        {
            ...
            public string? action { get; set; } // get | set_template | hide_category | show_category | set_scale | set_detail_level | set_discipline | set_phase | set_phase_filter | set_section_box | clear_section_box | set_crop_box | clear_crop_box | set_view_range | set_underlay | clear_underlay | set_category_override | clear_category_override | apply_view_filter | create_view_filter | remove_view_filter | clear_filter_override | isolate_elements_temp | isolate_categories_temp | clear_temp_hide_isolate | reveal_hidden_on | reveal_hidden_off | hide_elements | unhide_elements
        ...
        public Task<object> Handle(UIApplication app, string jsonData)
        {
            ...
            if (dryRun && action == "set_view_range")
            {
                ...
                return Task.FromResult<object>(new { status = "Dry Run", action, dryRun = true, current, proposed });
            }
            if (dryRun)
            {
        ...
        private static void ApplyAction(Document doc, View view, string action, Params p)
        {
            switch (action)
            {
                ...
                case "set_view_range":
                {
                    if (view is not ViewPlan plan) throw new InvalidOperationException("visibility.set_view_range requires a plan view.");
                    if (!HasViewRangeInput(p)) throw new InvalidOperationException("visibility.set_view_range requires at least one plane level or offset.");
