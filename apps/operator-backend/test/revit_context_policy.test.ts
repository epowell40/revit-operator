...
test("canonical and Sidecar compatibility envelopes both identify Revit turns", () => {
  ...
  assert.equal(hasRevitTurnContext({ revit: {} }), true);
  assert.equal(hasRevitTurnContext({ ui: { revit_document: {} } }), true);
  assert.equal(hasRevitTurnContext({ revit: null }), true);
  assert.equal(hasRevitTurnContext({ ui: { revit_document: null } }), true);
...
test("unscoped legacy memory is suppressed only for Revit turns", () => {
  ...
  assert.equal(mayInjectUnscopedLegacyMemory({ revit: {} }), false);
  assert.equal(mayInjectUnscopedLegacyMemory({ ui: { revit_document: {} } }), false);
});
