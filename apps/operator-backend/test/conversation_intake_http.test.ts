...
import { assertConversationIntakeResolved, retainedIntakeDecision } from "../src/conversation_intake.js";

test("HTTP unavailable classification cannot authorize task admission and an exact-request retry can recover",async t=>{
  process.env.OPERATOR_WORKSPACE_ROOT=fs.mkdtempSync(path.join(os.tmpdir(),"operator-intake-recovery-http-"));__closeForTests();
  let calls=0;
  const server=http.createServer((req,res)=>void handleConversationIntakeHttp(req,res,{
    ...
    interpreter:{interpret:async()=>{if(++calls===1)throw new Error("Provider temporarily unavailable");return {value:{route:"inspect",answer:null,question_kind:"current_model",identity_fields:[],read_evidence:"complete_collection",basis:"needs_tools",requested_effect:"read",entire_request_answered:false,confidence:0.99,reason:"Inspect the requested sheet collection."}};}}
  })).listen(0,"127.0.0.1");await once(server,"listening");
  t.after(()=>{server.close();__closeForTests();});
  const request={version:"operator.backend.v1",session_id:"owned",message_id:"sheets",user_text:"Inspect the sheet list and count the mechanical sheets. Reply in one sentence."};
  const url=`http://127.0.0.1:${(server.address() as any).port}`;
  const post=()=>fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(request)});
  const unavailable=await(await post()).json() as any;
  ...
  const recovered=await(await post()).json() as any;
  assert.equal(recovered.routing_status,"accepted");assert.equal(recovered.route,"inspect");
