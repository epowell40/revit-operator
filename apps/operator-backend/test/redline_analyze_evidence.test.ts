...
test("redline evidence preserves real camel-case annotation geometry, grounded route region, URI/hash, and page isolation", () => {
  ...
  assert.equal(JSON.stringify(evidence).includes("C:\\fixture"), false);
  assert.deepEqual(evidence.intent.proposed_actions, [{ tool: "/revit/sheets", body: { action: "detail", sheetNumber: "M-101", includePlacedViews: true, includeViewports: true, includeViewportGeometry: true, includeTitleBlocks: true, includeSheetOutline: true }, requires_apply: false }, { tool: "/revit/get-titleblock-info", body: { sheetNumber: "M-101" }, requires_apply: false }]); assert.deepEqual(base, legacy); assert.ok(evidence.verification.observed.every((gate) => gate.status === "not_run"));
});

test("redline evidence canonicalizes only the exact current analyzer sheet proposal", () => {
  for (const mutate of [(body: any) => delete body.includeTitleBlocks, (body: any) => body.includeViewports = false, (body: any) => body.extra = true, (body: any) => body.apply = false, (body: any) => body.dryRun = true, (body: any) => body.action = "list", (body: any) => body.sheetNumber = " M104 ", (body: any) => body.sheetNumber = "M104/../A001"]) { const response = structuredClone(base); mutate(response.suggested_revit_calls[0].body); assert.equal(adaptRedlineAnalyzeToAecIntentEvidence(response, options).intent.proposed_actions.some((proposal) => proposal.tool === "/revit/sheets"), false); }
  for (const mutate of [(call: any) => call.method = "GET", (call: any) => call.path = "/revit/delete"]) { const response = structuredClone(base); mutate(response.suggested_revit_calls[0]); assert.equal(adaptRedlineAnalyzeToAecIntentEvidence(response, options).intent.proposed_actions.some((proposal) => proposal.tool === "/revit/sheets"), false); }
});

test("redline sheet canonicalization rejects proxies and descriptor-unsafe bodies without aliasing", () => {
  const exact = () => structuredClone(base.suggested_revit_calls[0].body), from = (body: any) => ({ ...structuredClone(base), suggested_revit_calls: [{ ...structuredClone(base.suggested_revit_calls[0]), body }, structuredClone(base.suggested_revit_calls[1])] }), sheetActions = (body: any) => adaptRedlineAnalyzeToAecIntentEvidence(from(body), options).intent.proposed_actions.filter((proposal) => proposal.tool === "/revit/sheets");
  const happy = exact(), accepted = sheetActions(happy), replay = exact(); replay.includeSheetOutline = true; assert.deepEqual(accepted[0]?.body, { action: "detail", sheetNumber: "M-101", includePlacedViews: true, includeViewports: true, includeViewportGeometry: true, includeTitleBlocks: true, includeSheetOutline: true }); assert.deepEqual(sheetActions(replay)[0]?.body, accepted[0]?.body); happy.sheetNumber = "M105"; assert.equal((accepted[0]?.body as any).sheetNumber, "M-101");
  class SheetBody { action = "detail"; sheetNumber = "M-101"; includePlacedViews = true; includeViewports = true; includeViewportGeometry = true; includeTitleBlocks = true; }
  const nullPrototype = Object.assign(Object.create(null), exact()), symbolExtra = exact(), nonEnumerableExtra = exact(); symbolExtra[Symbol("extra")] = true; Object.defineProperty(nonEnumerableExtra, "extra", { value: true });
  let getterCount = 0; const returningGetter = exact(), throwingGetter = exact(); Object.defineProperty(returningGetter, "action", { enumerable: true, get: () => { getterCount += 1; return "detail"; } }); Object.defineProperty(throwingGetter, "action", { enumerable: true, get: () => { getterCount += 1; throw new Error("getter must not run"); } });
  const transparentProxy = new Proxy(exact(), {}), switchingProxy = new Proxy(exact(), { ownKeys: () => { throw new Error("ownKeys must not run"); } }), throwingProxy = new Proxy(exact(), { getPrototypeOf: () => { throw new Error("prototype trap must not run"); } });
  for (const body of [new SheetBody(), nullPrototype, symbolExtra, nonEnumerableExtra, returningGetter, throwingGetter, transparentProxy, switchingProxy, throwingProxy]) assert.doesNotThrow(() => assert.equal(sheetActions(body).length, 0));
