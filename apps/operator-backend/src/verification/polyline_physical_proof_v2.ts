...
export function polylinePhysicalProof({ points, segmentIds, fittingIds, profiles, system, endpointPolicy, rows }: {points:number[][];segmentIds:number[];fittingIds:number[];profiles:Row[];system:string;endpointPolicy:string;rows:Row[]}):boolean {
  ...
  for(let i=0;i<segmentIds.length-1;i++) {
    ...
    if(paired(segmentIds[i],left,segmentIds[i+1],right)) {
      if(!same(left.origin,points[i+1]))return false;
      continue;
    }
  ...
  if(used.size!==fittingIds.length)return false;
  for(const [owner,c,p] of ([[segmentIds[0],ordered[0][0],points[0]],[segmentIds.at(-1),ordered.at(-1)![1],points.at(-1)]] as [number,Row,number[]][])) {
    if(!same(c.origin,p))return false;
    const external=refs(c)!;
    if(external.length&&all.has(external[0].ownerId))return false;
