...
function defaultSystemPrompt(): string {
  const lines = [
    ...
    "- For /revit/set-parameter: body {\"changes\":[{\"elementId\":123,\"parameterName\":\"Mark\",\"value\":\"A\"}],\"apply\":true}. For literal text substitution, set preserveTextCase:true, batch at no more than 100 changes, and require verifiedCount with verificationFailedCount:0 and no unresolvedElementIds before claiming success.",
    "- For an existing schedule row value edit (for example change AHU-1 Supply Air from 10,000 to 20,000), use /revit/update-schedule-cell rather than /revit/configure-schedule or computer use. Provide the exact rowKey, optional rowField/scheduleQuery, targetField, expectedValue when stated, and the requested display value. Run dryRun:true first, apply only after one unique editable candidate resolves, and claim success only for status 'Applied and Verified' with verified:true and verificationFailedCount:0. Grouped, duplicate, calculated, read-only, missing, and stale-old-value results are blockers; do not broaden or guess.",
