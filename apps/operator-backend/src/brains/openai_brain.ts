...
function getRedlineFastPathState(sessionId: string): RedlineFastPathState {
  const now = Date.now();
...
function noteRedlineFastPathPhase(
  sessionId: string,
...
function appendRedlineFastPathCandidateDiagnostic(
  sessionId: string,
...
function buildRedlineFastPathDiagnosticsText(sessionId: string): string {
  const state = getRedlineFastPathState(sessionId);
...
function inferBridgeUrl(): string {
  return (process.env.REVIT_BRIDGE_URL ?? "http://localhost:5000").trim().replace(/\/+$/, "");
}

async function canUseDirectBridgeFastPath(): Promise<boolean> {
  ...
  let ok = false;
  try {
    const probe = await requestBridgeJson("GET", "/revit/context");
...
async function requestBridgeJson(method: "GET" | "POST", pathname: string, body?: unknown): Promise<unknown> {
  const token = getOrCreateOperatorToken();
  const controller = new AbortController();
  const timeoutMs = Math.max(2_000, Number.parseInt(process.env.OPERATOR_REDLINE_FAST_PATH_TIMEOUT_MS ?? "30000", 10) || 30_000);
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const headers: Record<string, string> = { "X-Operator-Token": token };
    if (body !== undefined) headers["Content-Type"] = "application/json";
    const res = await fetch(`${inferBridgeUrl()}${pathname}`, {
      method,
      headers,
...
function inferImageAttachmentMime(localPath: string): string {
...
function readAbsoluteImageDataUrl(localPath: string, maxBytes?: number | null): string | null {
  try {
...
function toToolResultFromDirectBridgeResult(result: DirectBridgeResult): ToolResult {
  const toolResult: ToolResult = compactIncomingToolResult({
