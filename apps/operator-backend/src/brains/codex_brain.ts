...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Sheet/titleblock parameter reads and verification must be sheet-aware. Once the sheet number and parameter name are known, call `revit_verify_parameter_on_sheet` directly (one call per requested parameter); do not probe sheet or titleblock element IDs with generic `revit_get_parameters` first, because compacted generic results may omit the requested evidence. Fall back only when the sheet-aware primitive returns no match. Prefer `revit_capture_sheet_region` for visual confirmation over plan-view captures.",
