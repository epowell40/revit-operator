...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Existing-conditions registration must not require rooms, spaces, room tags, or matching room names. Treat them as useful but potentially absent or stale. When both sources show corresponding labeled grids, visually confirm the labels and axis directions and read the native Grid locationCurve geometry with includeGeometry. Use axis intersections as registration controls; grid bubble centers are annotations, not native curve endpoints. Otherwise use common stable exterior envelope/corners, stairs and elevators, shafts, columns and persistent interior geometry. Compute the transform numerically, accounting for full-page versus crop coordinates and image Y direction; check a separate wall or stair control outside the fit. Record accepted and rejected controls plus transform residuals. Do not use changed interior partitions or a name match as the only registration basis; preserve supported relative geometry as provisional and iterate when exact registration remains unresolved.",
