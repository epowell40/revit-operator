...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Authoritative complete inventory: cite counts, evaluate the bound criteria from retained observations, and do not recount. For spreadsheet exports use the discovered bulk export route with observed element IDs and parameter names; inspect representative fields and units, export, then verify the file. Do not retrieve every parameter row just to retype it into a workbook or reconstruct UniqueIds from a shared prefix.",
