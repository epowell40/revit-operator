...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Duplicate-element investigation rule: do not treat repeated or unique Mark values or exact co-location as the definition of a duplicate, and do not rule duplicates out when those checks return zero. First request one bounded, project-scope `/revit/find-elements` inventory for the relevant physical category with `includeGeometry:true`; do not export every view before trying this complete inventory. Every document-scope `/revit/find-elements` request must include a real `category`/`categories`, `identityTerms`, or another supported search predicate; `limit` alone is not a bounded predicate. Use canonical `OST_...` category tokens when known; user-facing aliases such as `air terminals` are accepted by category-aware native endpoints. When `spatialDuplicateCandidates` is returned, inspect its ranked review groups first: unique Marks are not duplicate-instance proof, while opposite-facing peers may be intentional. Use insertion-point distance as well as bounding-box-center distance. Treat consecutive or nearby element IDs only as a creation-adjacency triage signal, never as duplicate proof. Batch every returned `candidateElementIds` through one `/revit/get-connectors` call with `includeAllRefs:true`, then compare connector/network signatures across the bounded candidates; rejecting one intentional pair is not evidence that unreviewed candidates are safe. If the summary is absent or incomplete, or custom ranking is needed, use code execution when available to group same-category and same-family/type instances and rank near-spatial candidates by overlapping bounding-box footprints or insertion-point/center separation relative to element size. Compare host, level, facing/hand orientation, parameters, and connector/network relationships; opposite-facing peers on different connector ports may be intentional. Shared-network membership may be intentional too, but neither fact alone proves that the pair is intentional or justifies skipping an explicitly requested rollback test. Before previewing deletion, trace both candidates and use a rollback/dry-run delete on the highest-ranked defensible candidate to report the exact disconnection and dependent-element effect. When the user explicitly requests that executable deletion-impact preview, a no-candidate conclusion does not satisfy the task unless every bounded candidate has genuinely been eliminated by stronger evidence and the assignment is truthfully reported incomplete. Use `/revit/export-visible-elements` only as a targeted visual follow-up for shortlisted candidates. If no candidate survives these checks, report the inspected category, inventory completeness, spatial thresholds, and how many bounded candidates were topology-reviewed.",
