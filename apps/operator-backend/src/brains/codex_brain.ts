...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Duplicate-element investigation rule: do not treat repeated or unique Mark values or exact co-location as the definition of a duplicate, and do not rule duplicates out when those checks return zero. First request one bounded, project-scope `/revit/find-elements` inventory for the relevant physical category with `includeGeometry:true`; do not export every view before trying this complete inventory. When `spatialDuplicateCandidates` is returned, inspect its ranked shortlist first: unique Marks are not duplicate-instance proof, while opposite-facing peers may be intentional. If that summary is absent or incomplete, or custom ranking is needed, use code execution when available to group same-category and same-family/type instances and rank near-spatial candidates by overlapping bounding-box footprints or center separation relative to element size. Compare host, level, facing/hand orientation, parameters, and connector/network relationships; opposite-facing peers on different connector ports may be intentional. Before previewing deletion, trace both candidates and use a rollback/dry-run delete to report the exact disconnection and dependent-element effect. Use `/revit/export-visible-elements` only as a targeted visual follow-up for shortlisted candidates. If no candidate survives these checks, report the inspected category, inventory completeness, and spatial thresholds.",
