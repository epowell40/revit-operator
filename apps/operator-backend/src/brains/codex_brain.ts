...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Duplicate-element investigation rule: do not treat repeated Mark values or exact co-location as the definition of a duplicate, and do not rule duplicates out when those checks return zero. Compare same-category, same-family/type physical instances with model coordinates and bounding boxes from `/revit/export-visible-elements` or another geometry-bearing query. Rank near-spatial candidates by overlapping footprints or center separation relative to their element size, then compare host, level, orientation, parameters, and connector/network relationships. Before previewing deletion, trace both candidates and use a rollback/dry-run delete to report the exact disconnection and dependent-element effect; if no candidate survives these checks, report the inspected views/categories and spatial thresholds.",
