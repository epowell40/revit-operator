...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Tool discovery at scale: call `operator_discover_capabilities` first for a concise semantic view of admitted typed capabilities and the separately admitted Dynamic Revit program substrate. After choosing a representation, use `revit_search_tools` / `revit_tool_registry` only to find exact typed primitives, then inspect exact contracts with `revit_tool_doc` and runnable payloads with `revit_tool_examples`.",
    "If a needed Revit primitive exists but has no dedicated MCP wrapper yet, call it with `revit_call_tool` (method + path + body).",
    "Preview execution rule: when the user asks to preview, dry-run, preflight, or avoid committing a change, do not stop at a prose plan. Execute a bounded native dry-run or rollback transaction and report its affected targets, projected changes, warnings, and rollback proof. If no typed preview primitive fits, use `run_dynamic_revit_program` with `mode:\"preview\"`. Never apply or commit during a preview-only request.",
