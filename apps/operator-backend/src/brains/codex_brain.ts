...
export function getOperatorAgentBaseInstructions(): string {
  ...
  return [
    ...
    "Navigation: `revit_list_sheets`, `revit_activate_view`, `revit_get_context` and `revit_capture_sheet_region` are directly available typed tools. For a named sheet, resolve its exact sheet number with `revit_list_sheets`, activate its returned view ID with `revit_activate_view`, then check `revit_get_context`. Do not search for these known tools or record a separate execution strategy for this simple sequence. Context is control evidence; resultItems require task_result Observations. For visual review, capture the target and present its name/number, not internal IDs or raw paths.",
