...
export function conversationWorkProfile(req: ChatRequest) {
  ...
  try {
    ...
    const evidenceInstruction = decision.read_evidence === "model_content"
      ? " Inspect actual model objects or systems with an appropriate native query, such as a scoped /revit/quantify or /revit/find-elements read. A filename, project number, view name, or other document label cannot establish discipline, system subtype or modeled contents. Use only distinctions supported by the inspected objects. Answer the requested distinction; do not add an unrequested subtype or exclusivity discussion. If the evidence does not settle the actual question, state the specific remaining uncertainty. The criterion requires model.content_observed."
    ...
    return { settings: { ...settings, reasoning_effort: "low" as const }, focused: true,
      instruction: "This is a focused inspection question selected by conversational intake. Obtain the minimum sufficient fresh evidence and answer the complete question concisely. Preserve all task authority and verification requirements. For a bounded presence or identity question, select exact observed scalar facts from raw_payload for resultItems; incomplete inventory.* projections cannot establish a complete inventory or absence. Do not expand a presence question into a project-wide inventory. Put the complete answer and any material uncertainty in assessment.overview, usually one short sentence. Follow the user's requested length and format. Add a second sentence only when needed to answer or qualify the actual question. Supporting findings and scope notes are available in Details; do not repeat them in the overview. Do not add generic caveats about work the user did not request. Broaden inspection when the evidence or the actual question requires it." + evidenceInstruction };
  } catch { return normal; }
}
