  ...
  if (snapshot.spec.result_delivery_required && !snapshot.result_delivery) {
    gaps.push({
      ...
      reason: snapshot.spec.result_assessment_required && snapshot.spec.requested_effect === "read"
        ? "Answer the user's actual question in plain language. Native resultItems alone do not answer an interpretive question. Call operator_evaluate_assignment_criteria with a few concise, retained resultItems and assessment: overview (the direct answer), findings (the brief explanation, with 1-based evidence_indices), limitations and questions. For an ordinary question use one low-priority finding; it renders as a plain answer. Put the conclusion and any essential uncertainty in the answer itself. Do not repeat the inventory as the answer or claim more than the cited evidence supports. This presentation does not create native proof or passing criteria."
