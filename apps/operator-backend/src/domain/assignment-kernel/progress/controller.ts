...
export function deriveProgressGapsV2(snapshot: AssignmentSnapshotV2): readonly ProgressGapV2[] {
  ...
  if (snapshot.spec.result_delivery_required && !snapshot.result_delivery) {
    gaps.push({
      ...
      reason: "A successful read is not the delivered answer. Inspect retained evidence and address every part of the original request. Call operator_evaluate_assignment_criteria with resultItems selecting concise native values (label, observationId, path). For an audit, review, comparison or gap list, also supply assessment: overview, prioritized findings citing 1-based evidence_indices into resultItems, limitations and up to three questions. Assessment is assistant interpretation, not native proof or engineering certification. Select scalars or small scalar arrays, not whole reports or inventories. The terminal answer is rendered from this delivery; a later free-text answer cannot replace it. Presentation cannot create semantic facts or passing criteria."
    });
  }
