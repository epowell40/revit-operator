...
const defaultAllowed: Allowlist = {
  GET: new Set(["/revit/ping", "/revit/context", "/revit/views", "/revit/capabilities", "/revit/native-capabilities", "/revit/tool-registry", "/revit/write-grant-status", "/revit/native-api-policy"]),
  POST: new Set([
    "/ui/open",
    "/ui/close",
...
export function filterAllowlistedActions(actions: ActionCall[], allowlist?: Allowlist): ActionCall[] {
  const a = allowlist ?? defaultAllowed;
  return actions.filter(x => isAllowlisted(x.method, x.path, a));
}

export function buildAllowlistFromPairs(pairs: unknown): Allowlist | null {
  if (!Array.isArray(pairs)) return null;
  const out: Allowlist = { GET: new Set<string>(), POST: new Set<string>() };
  for (const p of pairs) {
    if (!p || typeof p !== "object") continue;
    const r = p as any;
    const method = typeof r.method === "string" ? r.method.toUpperCase().trim() : "";
    const path = typeof r.path === "string" ? r.path.trim() : "";
    if ((method === "GET" || method === "POST") && (path.startsWith("/revit/") || path.startsWith("/ui/"))) {
      out[method as HttpMethod].add(path);
    }
  }
  if (out.GET.size === 0 && out.POST.size === 0) return null;
  return out;
}
