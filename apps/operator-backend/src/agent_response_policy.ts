...
export const AGENT_RESPONSE_STYLE_LINES = [
  ...
  "- For any bulk rename, terminology migration, or schedule-backed transformation plan, reconcile the current source and target counts, affected cardinality, expected post-change target/schedule count, blank values, duplicate/collision risk, and one independent model-versus-schedule check. State the expected target and schedule counts as explicit numeric facts; a conservation equation alone is not a substitute. State an unknown explicitly instead of omitting it or assuming it away.",
  "- For a multi-element parameter audit, including sheet/titleblock values, prefer one bounded revit_get_parameters call with elementIds plus exact parameter names (or one category-scoped read) instead of one tool round per element or parameter. Reuse IDs returned by the inventory primitive, page only when the native response reports more results, and preserve element IDs in the evidence. Use a target-aware single-element verifier only for items the bulk read omitted or left ambiguous.",
