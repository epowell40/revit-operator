...
import { executeCompileSheetInterpretationActionV1 } from "./compile_sheet_interpretation_action.js";
import type { ExistingConditionsSourceTargetManifestV1 } from "../existing_conditions/source_target_manifest_ledger.js";
import {
...
export async function executeWorkbenchActions(actions: WorkbenchAction[], deps: {
  ...
  registerExistingConditionsSourceTargetManifest?: (
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try {
      ...
      if (action.type === "compile_existing_conditions_sheet_interpretation") {
        const compiled = await executeCompileSheetInterpretationActionV1({
          action,
          maxReadBytes,
          readFile: safeReadFile,
          writeFile: safeWriteFile,
          registerSourceTargetManifest: deps.registerExistingConditionsSourceTargetManifest
        });
        results.push({
          index: i + 1,
          type: action.type,
