...
const schema = {
  ...
  required: ["schema", "sheet_numbers", "field_names", "find", "replace", "expected_value", "max_changes", "confidence", "evidence"],
  properties: {
    schema: { type: "string", enum: [SCHEDULE_VALUE_REPLACEMENT_INTENT_SCHEMA] },
    sheet_numbers: { type: "array", minItems: 1, maxItems: 50, items: { type: "string" } },
...
const instructions = `Extract one explicit Revit request to replace a literal substring in schedule-backed values on explicit sheet numbers.
Do not invent sheets, fields, find text, replacement text, exact old values, or limits.
sheet_numbers are only explicitly named drawing sheet numbers such as P6.01.
field_names are the explicitly named schedule column/parameter concepts. For DESIG/designation requests return ["DESIG","Designation"].
...
Use ambiguity=material unless at least one sheet, one field concept, and the exact find/replace pair are present.`;
...
export function parseDirectScheduleValueReplacement(userText: string): ScheduleValueReplacementIntentV1 | null {
  ...
  if (sheets.length === 0 || tokens.length < 2 || tokens[0] === tokens[1]) return null;
  ...
  return {
    ...
    confidence: {
      ...
      reasons: [single ? "Explicit one-item schedule replacement grammar matched." : "Explicit sheet-scoped literal schedule replacement grammar matched."]
    },
...
export function normalizeScheduleValueReplacementIntent(value: unknown, authoritativeUserText: string): ScheduleValueReplacementIntentV1 {
  ...
  if (sheetNumbers.length === 0 || fieldNames.length === 0 || find === replace) throw new Error("Incomplete schedule value replacement intent");
