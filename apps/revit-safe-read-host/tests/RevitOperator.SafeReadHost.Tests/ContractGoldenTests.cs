...
namespace RevitOperator.SafeReadHost.Tests
{
    public sealed class ContractGoldenTests
    {
        ...
        [Fact] public void Strict_responses_match_the_exact_mcp_envelopes()
        {
            using(JsonDocument ok=JsonDocument.Parse(ResponsePayload.Success(12).Body)){Assert.Equal(new[]{"schema","count"},ok.RootElement.EnumerateObject().Select(p=>p.Name));}using(JsonDocument fail=JsonDocument.Parse(ResponsePayload.Failure(CertifiedFailureCode.DeadlineExceeded,CertifiedExecutionPhase.CountSheets,true,true).Body)){Assert.Equal(new[]{"schema","code","error","retryable","request_dispatched","outcome_unknown","phase"},fail.RootElement.EnumerateObject().Select(p=>p.Name));Assert.True(fail.RootElement.GetProperty("request_dispatched").GetBoolean());Assert.True(fail.RootElement.GetProperty("outcome_unknown").GetBoolean());Assert.False(fail.RootElement.GetProperty("retryable").GetBoolean());}
        }
        [Fact] public void Backend_failure_semantics_survive_the_host_envelope()
        {
