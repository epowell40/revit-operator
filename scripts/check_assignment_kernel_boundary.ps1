...
foreach ($file in $domainFiles) {
  ...
  foreach ($match in [regex]::Matches($content, '(?m)^\s*(?:import|export)\b[^\r\n]*?from\s+["'']([^"'']+)["'']')) {
    ...
    # The reducer also validates the shared native artifact receipt; this package has no transport or I/O dependencies.
    $sharedControlEvidenceContract = $specifier -eq "@revitoperator/assignment-kernel-v2-contracts" -and @("operation.ts", "progress/execution_failure.ts", "progress/provider_call.ts", "reducer.ts", "semantic_admissibility.ts", "snapshot.ts") -contains $relative
