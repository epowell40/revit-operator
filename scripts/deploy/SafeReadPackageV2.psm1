...
function Get-SafeReadProofArtifact {
  ...
  if ([int]$receipt.schemaVersion -ne 1 -or $receipt.proofKind -cne $script:ProofKind -or $receipt.mode -cne 'check' -or $receipt.status -cne 'verified' -or -not [bool]$receipt.certified -or @($receipt.issues).Count -ne 0) { throw "SafeRead proof receipt is not certified and verified: $receiptPathResolved" }
...
function Assert-SafeReadBundle {
  ...
  foreach ($target in $targets) {
    ...
    if([int]$proofReceipt.schemaVersion -ne 1 -or $proofReceipt.proofKind -cne $script:ProofKind -or $proofReceipt.mode -cne 'check' -or $proofReceipt.status -cne 'verified' -or -not [bool]$proofReceipt.certified -or @($proofReceipt.issues).Count -ne 0){throw "SafeRead target $year preserved proof receipt is not certified."}
